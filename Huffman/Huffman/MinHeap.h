...
{
private:
	T*heap;//元素数组，0号位置也存储元素
	int CurrentSize;//目前元素的个数
...
template<class T>void MinHeap<T>::FilterDown(const int start, const int end)
{
	//自下往上调整，使关键字小的节点在上面
	int i = start, j = 2 * i + 1;
	T temp = heap[i];
...
template<class T>T MinHeap<T>::RemoveMin()
{
	...
	CurrentSize--;
	FilterDown(0, CurrentSize - 1);//调整新的节点
	return x;
}
...
template<class T>bool MinHeap<T>::IsEmpty()const
{
