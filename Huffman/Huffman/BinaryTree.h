...
template<class T>void BinaryTree<T>::In_Order()
{
	InOrder(root);
}
template<class T>void BinaryTree<T>::Post_Order()
{
	PostOrder(root);
}
...
template<class T>void BinaryTree<T>::DestroyTree()
{
	Destroy(root);
}
template<class T>void BinaryTree<T>::PreOrder(BTNode<T> *r)
{
	if (r != NULL)
	{
		cout << r->data << ' ';
...
template<class T>void BinaryTree<T>::InOrder(BTNode<T>*r)
{
	if (r != NULL)
	{
		InOrder(r->lChild);
...
template<class T>void BinaryTree<T>::PostOrder(BTNode<T>*r)
{
	if (r != NULL)
	{
		PostOrder(r->lChild);
...
template<class T>int BinaryTree<T>::NodeCount(const BTNode<T>*r)const
{
	if (r == NULL)
		return 0;
	else
...
template<class T>int BinaryTree<T>::Height(const BTNode<T>*r)const
{
	if (r == NULL)
	{
		return 0;
...
template<class T>void BinaryTree<T>::Destroy(BTNode<T>*&r)
{
	if (r != NULL)
	{
		Destroy(r->lChild);
...
template<class T>void BinaryTree<T>::Change(BTNode<T>*r)//将二叉树所有的节点的左右子树交换
{
	BTNode<T>*p;
	if (r)
	{
...
template<class T>void BinaryTree<T>::MakeTree(T pData, BinaryTree<T>leftTree, BinaryTree<T>rightTree)
{
	root = new BTNode<T>();
	root->data = pData;
